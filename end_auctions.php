...
$one = "1";
$stmt = $conn->prepare("SELECT id, user_id, auction_type FROM product_info WHERE bid_placed = ?");
...
while($row = mysqli_fetch_array($result,MYSQLI_ASSOC))
{
  ...
  if(strcmp($row['auction_type'], $vickery) == 0)
  {
	...
	$stmt = $conn->prepare("SELECT * FROM bidding_info WHERE product_id = ? ORDER BY amount DESC LIMIT 1, 1");
	...
	while($row2 = mysqli_fetch_array($result2,MYSQLI_ASSOC))
	{
      ...
      $stmt = $conn->prepare("SELECT acc_balance FROM user_info WHERE username = ?");
	  $stmt->bind_param("s", $row5['bidder_id']);
	  $stmt->execute();
	  ...
	  echo "The new account of the buyer " . $accMinus . " by " . $row5['bidder_id'] . "<br>";
     
      $stmt = $conn->prepare("UPDATE user_info SET acc_balance = (?) WHERE username = ?");
      $stmt->bind_param("is",$accMinus, $row5['bidder_id']);
      $stmt->execute();	

      $stmt = $conn->prepare("UPDATE product_info SET boughtby = ? WHERE id = ?");
	  $stmt->bind_param("si", $row5['bidder_id'], $row['id']);
	  $stmt->execute();

	  $stmt = $conn->prepare("INSERT INTO winner_info (product_id, winner_id, amount) VALUES (?, ?, ?)");
      $stmt->bind_param("isi",$row['id'] ,$row5['bidder_id'], $row2['amount']);
      $stmt->execute();	
